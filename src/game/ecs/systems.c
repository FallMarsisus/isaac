...
uint32_t add_player(ECS_Manager* ecs) {
    ...
    RenderComponent* render = ECS_AddComponent(ecs, player, RENDER, sizeof(RenderComponent));
    ...
    render->width = 50; render->height = 50;
...
uint32_t add_enemy(ECS_Manager* ecs, uint32_t player) {
    ...
    RenderComponent* render = ECS_AddComponent(ecs, enemy, RENDER, sizeof(RenderComponent));
        ...
        if (position) {
            ...
            if(targetComp) {
                ...
                if(target_pos) {
                    ...
                    position->vx = dx / distance;
                    position->vy = dy / distance;
                }
            }
            
            position->x += position->vx; // Assuming 60 FPS
